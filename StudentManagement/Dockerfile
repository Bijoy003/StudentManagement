...
WORKDIR /src
COPY ["StudentManagement/StudentManagement.Web.csproj", "StudentManagement/"]
...
RUN dotnet restore "./StudentManagement/StudentManagement.Web.csproj"
COPY . .
WORKDIR "/src/StudentManagement"
RUN dotnet build "./StudentManagement.Web.csproj" -c $BUILD_CONFIGURATION -o /app/build

# This stage is used to publish the service project to be copied to the final stage
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./StudentManagement.Web.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
